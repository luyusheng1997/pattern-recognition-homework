...
df["photo_num"]=df["photos"].apply(lambda x:len(x.split(",")) if x!="[]" else 0)#map(lambda x:len(x.split(",")) if x!="[]" else 0,df["photos"])
df["feature_num"]=df["features"].apply(lambda x:len(x.split(",")) if x!="[]" else 0)#map(lambda x:len(x.split(",")) if x!="[]" else 0,df["features"])

a=40.705628
b=-74.010278
#distance to the doc select
df["distance"]=df[["longitude","latitude"]].apply(lambda x:int(math.sqrt((x[0]-b)**2+(x[1]-a)**2)*111),axis=1)#map(lambda x,y:int(math.sqrt((x-b)**2+(y-a)**2)*111),df["longitude"],df["latitude"])
...
def time_long(x,y):
    ...
    if x==6:
        return 30+31+y
df["time"]=df[["created_month","created_day"]].apply(lambda x:time_long(x[0],x[1]),axis=1)#map(lambda x,y:time_long(x,y),df["created_month"],df["created_day"])
...
df["display_count"]=df["display_address"].apply(lambda x:display[x])#map(lambda x:display[x],df["display_address"])
df["manager_count"]=df["manager_id"].apply(lambda x:manager_id[x])#map(lambda x:manager_id[x],df["manager_id"])    #经理所发的数目
df["building_count"]=df["building_id"].apply(lambda x:building_id[x])#map(lambda x:building_id[x],df["building_id"])
df["street_count"]=df["street_address"].apply(lambda x:street[x])#map(lambda x:street[x],df["street_address"])
df["bedrooms_count"]=df["bedrooms"].apply(lambda x:bedrooms[x])#map(lambda x:bedrooms[x],df["bedrooms"])
df["bathrooms_count"]=df["bathrooms"].apply(lambda x:bathrooms[x])#map(lambda x:bathrooms[x],df["bathrooms"])
df["day_count"]=df["time"].apply(lambda x:days[x])#map(lambda x:days[x],df["time"])
...
df["manager_place"]=df["manager_id"].apply(lambda x:managet_place[x])#map(lambda x:managet_place[x],df["manager_id"])
...
#每个经纬度分类一下（1平方千米一个类）
df["jwd_class"]=df[["latitude","longitude"]].apply(lambda x:(int(x[0]*100)%100)*100+(int(-x[1]*100)%100))#map(lambda x,y:(int(x*100)%100)*100+(int(-y*100)%100),df["latitude"],df["longitude"])

df=df.drop(["photos","created"],axis=1)
